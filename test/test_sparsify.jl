...
symB = SymmetricBasis(ACEfrictionCore.Invariant(), B1p, Bsel)
...
ACEfrictionCore.sparsify!(symB2, keep = keep)
...
for ntest = 1:30 
   local cfg , BB1 
   cfg = [ State(rr = ACEfrictionCore.rand_vec3(B1p["Rn"])) for _=1:10 ] 
