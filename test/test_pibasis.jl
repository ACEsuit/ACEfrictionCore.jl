...
using ACEfrictionCore, Random
using Printf, Test, LinearAlgebra, ACEfrictionCore.Testing, StaticArrays
...
for (i, b1) in enumerate(ACEfrictionCore.get_spec(B1p))
  inv_spec1[b1] = i
end
...
# AA, dAA = ACEfrictionCore.evaluate_ed(pibasis, cfg)
...
#   local AA, dAA 
#   AA1, dAA = ACEfrictionCore.evaluate_ed(pibasis, cfg)
...
#     F = t -> sum(c .* ACEfrictionCore.evaluate(pibasis, ACEConfig(Xs + t[1] * Us)))
#     dF = t -> [ Us' * _rrval.(sum(c .* ACEfrictionCore.evaluate_ed(pibasis, ACEConfig(Xs + t[1] * Us))[2], dims=1)[:]) ]
...
# import ACEfrictionCore: evaluate_ed 
...
# A, dA = ACEfrictionCore.evaluate_ed(pibasis.basis1p, cfg)
...
# @btime ACEfrictionCore.evaluate_ed!($AA, $dAA, $pibasis, $Xs)

# @profview begin 
#   for _=1:100 
#     ACEfrictionCore.evaluate_ed!(AA, dAA, pibasis, Xs)
#   end 
# end